...
- Attention:
    ...
    - All four wrapper attention routines above accept `--backends prims-ts` on SM100/SM103 to benchmark the experimental task-scheduled attention implementation. The standalone `trtllm_batch_decode_sparse_mla_dsv4` routine supports only `trtllm-gen`.
- GEMM:
